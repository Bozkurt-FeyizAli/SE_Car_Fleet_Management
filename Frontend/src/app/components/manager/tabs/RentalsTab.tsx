...
export function RentalsTab() {
  const compRentals = () => interCompanyRentals.filter(r => r.owner_comp_id === currentCompany.id || r.renter_comp_id === currentCompany.id);
  const [data, setData] = useState(compRentals());
  ...
  const [form, setForm] = useState({ renter_comp_id: "", vehicle_id: "", driver_id: "", dynamic_price: "3000", start_date: "", end_date: "", status: "active" as InterCompanyRental["status"] });

  const openAdd = () => { setForm({ renter_comp_id: "", vehicle_id: "", driver_id: "", dynamic_price: "3000", start_date: "", end_date: "", status: "active" }); setEditItem(null); setShowForm(true); };
  const openEdit = (item: InterCompanyRental) => { setForm({ renter_comp_id: item.renter_comp_id.toString(), vehicle_id: item.vehicle_id.toString(), driver_id: item.driver_id?.toString() ?? "", dynamic_price: item.dynamic_price.toString(), start_date: item.start_date, end_date: item.end_date ?? "", status: item.status }); setEditItem(item); setShowForm(true); };

  const handleSave = () => {
    if (!form.renter_comp_id || !form.vehicle_id) { toast.error("Zorunlu alanlari doldurun"); return; }
    if (editItem) {
      const idx = interCompanyRentals.findIndex(r => r.id === editItem.id);
      if (idx >= 0) interCompanyRentals[idx] = { ...interCompanyRentals[idx], renter_comp_id: Number(form.renter_comp_id), vehicle_id: Number(form.vehicle_id), driver_id: form.driver_id ? Number(form.driver_id) : null, dynamic_price: Number(form.dynamic_price), start_date: form.start_date, end_date: form.end_date || null, status: form.status };
    } else {
      interCompanyRentals.push({ id: nextId(), owner_comp_id: currentCompany.id, renter_comp_id: Number(form.renter_comp_id), vehicle_id: Number(form.vehicle_id), driver_id: form.driver_id ? Number(form.driver_id) : null, dynamic_price: Number(form.dynamic_price), start_date: form.start_date, end_date: form.end_date || null, status: form.status });
    }
    setData(compRentals());
    setShowForm(false);
    toast.success(editItem ? "Kiralama guncellendi" : "Kiralama eklendi");
  };

  const handleDelete = () => {
    if (!deleteItem) return;
    const idx = interCompanyRentals.findIndex(r => r.id === deleteItem.id);
    if (idx >= 0) interCompanyRentals.splice(idx, 1);
    setData(compRentals());
    setDeleteItem(null);
  ...
  const columns: Column<InterCompanyRental>[] = [
    ...
    { key: "price", header: "Fiyat", render: (r) => `₺${r.dynamic_price.toLocaleString("tr-TR")}` },
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Arac *">
            <select className="w-full h-9 rounded-md border border-border bg-input-background px-3 text-sm" value={form.vehicle_id} onChange={e => setForm({ ...form, vehicle_id: e.target.value })}>
              <option value="">Sec...</option>
              {vehicles.filter(v => v.company_id === currentCompany.id).map(v => <option key={v.id} value={v.id}>{v.plate_number}</option>)}
            </select>
          </Field>
          <Field label="Sofor">
            <select className="w-full h-9 rounded-md border border-border bg-input-background px-3 text-sm" value={form.driver_id} onChange={e => setForm({ ...form, driver_id: e.target.value })}>
              <option value="">Sec...</option>
              {drivers.map(d => <option key={d.id} value={d.id}>{d.first_name} {d.last_name} (Puan: {d.current_score})</option>)}
            </select>
          </Field>
          <Field label="Fiyat (TL)"><Input type="number" value={form.dynamic_price} onChange={e => setForm({ ...form, dynamic_price: e.target.value })} /></Field>
          <Field label="Baslangic"><Input type="date" value={form.start_date} onChange={e => setForm({ ...form, start_date: e.target.value })} /></Field>
          <Field label="Bitis"><Input type="date" value={form.end_date} onChange={e => setForm({ ...form, end_date: e.target.value })} /></Field>
        </div>
      </FormDialog>
