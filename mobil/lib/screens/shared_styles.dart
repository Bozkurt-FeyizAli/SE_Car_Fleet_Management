...
Widget kField(String label, TextEditingController c,
    {TextInputType? type, bool obscure = false, String? hint}) {
  return Padding(
...
Color kStatusColor(String? s) {
  switch (s) {
    ...
    case 'on_trip':   return const Color(0xFF60A5FA);
    case 'off_duty':  return const Color(0xFFA78BFA);
...
String kStatusLabel(String? s) {
  switch (s) {
    ...
    case 'on_trip':   return 'Seferde';
    case 'off_duty':  return 'İzinli';
...
// ─── Dropdown field ───────────────────────────────────────────────────────
Widget kDropdown(String value, List<String> values, List<String> labels,
...
// ─── TextField with hint ──────────────────────────────────────────────────
Widget kFieldHint(String label, TextEditingController c,
    {TextInputType? type, bool obscure = false, String? hint}) {
  return Padding(
    padding: const EdgeInsets.only(bottom: 10),
    child: TextField(
      ...
      obscureText: obscure,
      style: const TextStyle(color: Colors.white, fontSize: 14),
      decoration: fieldDecor(label).copyWith(hintText: hint, hintStyle: const TextStyle(color: kMuted, fontSize: 13)),
    ),
  );
}
...
Widget kActions(VoidCallback onEdit, VoidCallback onDelete) => Row(
  ...
  children: [
    TextButton.icon(
      ...
      label: const Text('Düzenle', style: TextStyle(color: kBlue, fontSize: 12)),
