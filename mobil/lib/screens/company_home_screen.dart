...
// ─── 9 tabs matching web ManagerPanel ─────────────────────────────────────
const _tabs = [
...
class CompanyHomeScreen extends StatefulWidget {
  ...
  const CompanyHomeScreen({super.key, required this.user, required this.authService});
  @override
...
class _State extends State<CompanyHomeScreen> {
  ...
  Widget _body() {
    switch (_tabs[_idx].id) {
      case 'users':     return const CompanyUsersTab();
      ...
      case 'orders':    return const CompanyOrdersTab();
      case 'payments':  return const CompanyPaymentsTab();
      case 'docs':      return const CompanyDocumentsTab();
      case 'settings':  return const _CompanySettingsTab();
      default:          return _CompanyDashboard(user: widget.user);
    }
  }
        ...
        child: Row(children: [
          ...
          Container(width: 32, height: 32,
              decoration: BoxDecoration(
                  color: kBlue.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(16)),
  ...
  Widget _bottomNav(BuildContext context) {
    ...
    return Container(
      ...
      child: SafeArea(child: SizedBox(height: 58,
        child: Row(children: List.generate(_tabs.length, (i) {
          final t = _tabs[i];
          final sel = _idx == i;
          return GestureDetector(
            onTap: () => setState(() => _idx = i),
            child: Container(width: w, padding: const EdgeInsets.symmetric(vertical: 6),
              color: Colors.transparent,
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(t.icon, size: 21, color: sel ? kBlue : kMuted),
                const SizedBox(height: 2),
                Text(t.label, maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center,
                    style: TextStyle(fontSize: 9, color: sel ? kBlue : kMuted,
...
class _CompanyDashboard extends StatefulWidget {
  ...
  @override State<_CompanyDashboard> createState() => _DashState();
}
...
class _DashState extends State<_CompanyDashboard> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() {
        _drivers = allUsers.where((u) => (u['roleId'] as num?)?.toInt() == 3).toList();
  ...
  Color _scoreColor(int s) => s >= 80 ? kGreen : s >= 60 ? Colors.orange : Colors.red;

  @override
  Widget build(BuildContext context) {
    final onTrip      = _drivers.where((d) => d['driverTripStatus'] == 'on_trip').length;
    final activeV     = _vehicles.where((v) => v['isActive'] == true).length;
    final inactiveV   = _vehicles.length - activeV;
    final scores      = _drivers.map((d) => (d['driverScore'] as num?)?.toInt() ?? 0);
    final avgScore    = _drivers.isEmpty ? 0.0 : scores.reduce((a, b) => a + b) / _drivers.length;
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (_company != null) ...[
            ...
            const SizedBox(height: 2),
            Text(
              [_company!['address'], _company!['phone']].where((v) => v != null && v.toString().isNotEmpty).join(' · '),
          ...
          if (_loading)
            ...
            GridView.count(
              ...
              children: [
                _stat(Icons.local_shipping_rounded, 'Şoförler',    _drivers.length,
                    sub: '$onTrip seferde',             color: kGreen),
                _stat(Icons.directions_car_rounded,  'Araçlar',    _vehicles.length,
                    sub: '$activeV aktif',              color: kBlue),
                _stat(Icons.trending_up_rounded,     'Aktif Seferler', onTrip,
                    sub: 'Ort. puan: ${avgScore.toStringAsFixed(1)}', color: const Color(0xFF7C3AED)),
                _stat(Icons.shopping_cart_rounded,   'Siparişler', 0,
                    sub: '0 beklemede',                color: const Color(0xFFEA580C)),
                _stat(Icons.credit_card_rounded,     'Gelir',      null,
                    label2: '₺0',                      color: const Color(0xFF0D9488)),
                _stat(Icons.credit_card_rounded,     'Gider',      null,
                    label2: '₺0',                      color: const Color(0xFFEF4444)),
                _stat(Icons.swap_horiz_rounded,      'Kiralamalar', activeRent,
                    sub: 'aktif',                      color: const Color(0xFF0891B2)),
                _stat(Icons.warning_amber_rounded,   'Devre Dışı Araçlar', inactiveV,
            ...
            Container(
              ...
              child: const Padding(
                ...
                child: Center(
                  child: Text('Bu şirkete ait sipariş bulunamadı.',
            ...
            if (_drivers.isEmpty)
              Container(
                ...
                decoration: BoxDecoration(color: kCard, borderRadius: BorderRadius.circular(10),
                    border: Border.all(color: kBorder)),
                child: const Center(child: Text('Şoför kaydı yok.', style: TextStyle(color: kMuted))),
              )
            else
              Column(children: _drivers.map((d) {
                ...
                return Container(
                  ...
                  child: Row(children: [
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      const SizedBox(height: 2),
                      kBadge(kStatusLabel(status), kStatusColor(status)),
                    ])),
                    Text('$score',
                        style: TextStyle(color: _scoreColor(score), fontSize: 22, fontWeight: FontWeight.w800)),
                  ]),
                );
              }).toList()),
  ...
  Widget _stat(IconData icon, String label, int? value,
      {String? sub, String? label2, required Color color}) {
    return Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(color: kCard, borderRadius: BorderRadius.circular(10),
          border: Border.all(color: kBorder)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start,
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (sub != null) Text(sub, style: TextStyle(color: color, fontSize: 10)),
        ]),
      ]),
    );
  }
...
class _CompanySettingsTab extends StatefulWidget {
  ...
  @override State<_CompanySettingsTab> createState() => _SettState();
}
...
class _SettState extends State<_CompanySettingsTab> {
  ...
  @override
  Widget build(BuildContext context) {
    return SingleChildScrollView(padding: const EdgeInsets.all(14),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Ayarlar',
  ...
  Widget _card(IconData icon, String title, List<Widget> children) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Icon(icon, color: kMuted, size: 16), const SizedBox(width: 8),
